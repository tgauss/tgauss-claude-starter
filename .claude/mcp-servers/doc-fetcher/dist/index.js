...
import { listCached, storeDocumentation, cleanupOldVersions, } from './storage.js';
...
const FetchDocumentationSchema = z.object({
    ...
    topics: z.array(z.string()).optional().describe('Specific topics to fetch (optional, fetches all if omitted)'),
});
/**
 * Tool 3: list_cached
...
server.setRequestHandler(ListToolsRequestSchema, async () => {
    return {
        tools: [
            {
                name: 'search_docs',
                description: 'Search for available documentation topics across tracked packages. Returns matching topics with their cache status.',
                inputSchema: {
                    type: 'object',
                    properties: {
                        query: {
            ...
            },
            {
                ...
                description: 'Fetch and cache documentation for a specific package and version. Fetches from GitHub, filters content (500KB→50KB), and stores locally. Returns summary, not full content (MCP best practice).',
                inputSchema: {
                    type: 'object',
                    properties: {
                        package: {
                        ...
                        topics: {
                            type: 'array',
                            items: { type: 'string' },
                            description: 'Specific topics to fetch (optional, fetches all configured topics if omitted)',
                        },
                    },
            ...
            {
                name: 'list_cached',
                description: 'List all cached documentation with versions, topics, and sizes. Shows what\'s available offline.',
                inputSchema: {
                    type: 'object',
                    properties: {
                        package: {
    ...
    try {
        switch (name) {
            ...
            case 'fetch_documentation': {
                ...
                if (!packageConfig) {
                    throw new Error(`Package "${packageName}" not in curated list. Available: ${sources.trackedPackages.map((p) => p.name).join(', ')}`);
                }
                ...
                if (invalidTopics.length > 0) {
                    throw new Error(`Invalid topics for ${packageName}: ${invalidTopics.join(', ')}. Available: ${packageConfig.topics.join(', ')}`);
                }
                ...
                for (const topic of topicsToFetch) {
                    try {
                        ...
                        // Store locally
                        const storagePath = await storeDocumentation(packageName, version, topic, filtered, packageConfig.source);
                        results.push({
                            topic,
                            success: true,
                            path: storagePath,
