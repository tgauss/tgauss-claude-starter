...
export async function fetchDocumentation(
  ...
  const url = constructGitHubRawUrl(
    packageConfig.source.repo,
    packageConfig.source.branch,
    path,
    version
  );
  ...
  try {
    ...
    throw new Error(
      `Failed to fetch documentation from ${url}: ${error instanceof Error ? error.message : 'Unknown error'}`
    );
  }
}
...
function removeNoisePatterns(content: string, removeElements: string[]): string {
  ...
  if (removeElements.includes('nav')) {
    filtered = filtered.replace(
      /^#{1,6}\s*(Navigation|Menu|Table of Contents)[\s\S]*?(?=^#{1,6}\s|\n\n)/gim,
      ''
    );
  }
  ...
  if (removeElements.includes('related-articles')) {
    filtered = filtered.replace(
      /^#{1,6}\s*(Related|See Also|Further Reading)[\s\S]*?(?=^#{1,6}\s|\n\n)/gim,
      ''
    );
  }
...
function limitCodeExamples(content: string, maxExamples: number): string {
