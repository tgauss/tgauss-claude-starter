...
export async function storeDocumentation(
  packageName: string,
  version: string,
  topic: string,
  content: string,
...
async function updateRegistry(
  ...
  registry.stats.totalVersions = Object.values(registry.packages).reduce(
    (sum, pkg) => sum + pkg.versions.length,
    0
  );
  registry.stats.totalTopics = Object.values(registry.packages).reduce(
    (sum, pkg) => sum + pkg.topics.length,
    0
  );
...
async function calculateTotalSize(): Promise<number> {
  ...
  try {
    ...
    for (const pkg of packages) {
      if (pkg === '_registry.json') continue;
...
async function calculatePackageSize(packageName: string): Promise<number> {
  let packageSize = 0;
  const normalized = normalizePackageName(packageName);
...
export async function getMetadata(packageName: string, version: string): Promise<any | null> {
  const normalized = normalizePackageName(packageName);
...
export async function listCached(): Promise<Registry> {
  if (!existsSync(REGISTRY_PATH)) {
    return {
      version: '1.0.0',
      lastUpdated: null,
...
export async function cleanupOldVersions(packageName: string, maxVersions: number): Promise<void> {
  const normalized = normalizePackageName(packageName);
  const pkgPath = join(KNOWLEDGE_BASE_PATH, normalized);
